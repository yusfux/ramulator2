    ...
    for org in org_list:
      for row_policy in row_policy_list:
        for trace in trace_list:
          sbatch_filename    = f'{WORK_DIR}/run_scripts/{device}_{org}_{timing_list[device]}_{row_policy}_{trace}.sh'
          config_filename    = f'{RESULT_DIR}/configs/{device}_{org}_{timing_list[device]}_{row_policy}_{trace}.yaml'
          result_filename    = f'{RESULT_DIR}/stats/{device}_{org}_{timing_list[device]}_{row_policy}_{trace}.yaml'
          error_filename     = f'{RESULT_DIR}/errors/{device}_{org}_{timing_list[device]}_{row_policy}_{trace}.txt'
          cmd_count_filename = f'{RESULT_DIR}/cmd_counts/{device}_{org}_{timing_list[device]}_{row_policy}_{trace}.txt'
          wr_count_filename  = f'{RESULT_DIR}/wr_counts/{device}_{org}_{timing_list[device]}_{row_policy}_{trace}.txt'
          ...
          sbatch_cmd += f' --error={error_filename} --partition={PARTITION_NAME} --job_name"ramulator2" {sbatch_filename}'
...
for path in [
        ...
        f'{RESULT_DIR}/cmd_count',
        f'{RESULT_DIR}/wr_count'
    ]:
...
commands = get_singlecore_run_commands()
